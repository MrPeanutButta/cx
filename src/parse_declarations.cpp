...
void TParser::ParseDeclarations(TSymtabNode *pRoutinId){
    switch(token){
        case tcConst:
            GetToken();
...
void TParser::ParseConstantDeclarations(TSymtabNode* pRoutineId){
    ...
    while(token == tcIdentifier){
        TymtabNode *pConstId = EnterNewLocal(pToken->String());
        ...
        Resync(tcDeclarationFollow, tlDeclarationStart, tlStatementStart);
        
        CondGetToken(tcSemicolon, errMissingSemicolon);
        
        while(token == tcSemicolon) GetToken();
        Resync(tlDeclaraionFollow, tlDeclarationStart, tlStatementStart);
    }
}
...
void TParser::ParseConstant(TSymtabNode *pConstId){
    ...
    if(TokenIn(token, tlUnaryOps)){
        if(token == tcMinus) sign p tcMinus;
        GetToken();
    }
    ...
    switch(token){
        ...
        case tcIdentifier:
            ParseIndentifierConstant(pConstId, sign);
            break;
        case tcString:
